...
const CreateCharacterAppearance = ({ project, character, onClose }) => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        const response = await apiCall('POST', `/projects/${project.id}/characters/${character.id}/appearances/single`, form)
        if (response?.success) {
            pushToast(response.message || 'Aparición creada', 'success')
    ...
    return (
        <section className={styles.container}>
            <header className={styles.header}>
                <div className={styles.title}>
                    <Icon
                        ...
                        height={15}
                    />
                    <h3>Nueva aparición en libro</h3>
                </div>
                <p>Añade una aparición de &quot;{character.name}&quot; en uno de los libros del proyecto.</p>
            </header>
            ...
            <footer className={styles.footer}>
                ...
                <button className={styles.buttonSubmit} type='button' onClick={handleSubmit} disabled={loading || !form.book_id}>
                    {loading ? <Loader color='foreground' size={15} /> : <span>Crear aparición</span>}
                </button>
            </footer>
        </section>
    )
...
export default CreateCharacterAppearance
