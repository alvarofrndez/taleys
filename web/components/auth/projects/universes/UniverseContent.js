...
import styles from '@/assets/auth/projects/universes/content.module.scss'
...
export default function UniverseContent({ project, universe }) {
    ...
    const getAllItems = () => {
        const all_items = [
            ...(universe.universes || []).map(u => ({ ...u, type: 'universe', name: u.name, description: u.description || '' })),
            ...(universe.sagas || []).map(s => ({ ...s, type: 'saga', name: s.name, description: s.description || '' })),
            ...(universe.books || []).map(b => ({ ...b, type: 'book', name: b.title, description: b.description || '' })),
            ...(universe.characters || []).map(c => ({ ...c, type: 'character', name: c.name, description: c.biography || '' })),
        ]
    ...
    const filtered_items = useMemo(() => getAllItems(), [universe, search_term, active_filters])
