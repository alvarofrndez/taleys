...
import styles from '@/assets/auth/projects/sagas/content.module.scss'
...
export default function SagaContent({ project, saga }) {
    ...
    const getAllItems = () => {
        const all_items = [
            ...(saga.universes || []).map(u => ({ ...u, type: 'universe', name: u.name, description: u.description || '' })),
            ...(saga.sagas || []).map(s => ({ ...s, type: 'saga', name: s.name, description: s.description || '' })),
            ...(saga.books || []).map(b => ({ ...b, type: 'book', name: b.title, description: b.description || '' })),
            ...(saga.characters || []).map(c => ({ ...c, type: 'character', name: c.name, description: c.biography || '' })),
        ]
    ...
    const filtered_items = useMemo(() => getAllItems(), [saga, search_term, active_filters])
