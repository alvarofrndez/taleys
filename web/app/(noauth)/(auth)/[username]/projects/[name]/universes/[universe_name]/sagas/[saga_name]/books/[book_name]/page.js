...
export default function SagaUniverseViewPage() {
    ...
    useEffect(() => {
        async function fetchSaga() {
            ...
            const response = await apiCall('GET', `/projects/${project.id}/books/universes/${universe_name}/sagas/${saga_name}/books/name/${book_name}`)
            if (response.success) {
    ...
    return (
        <BookView book={book} universe_name={universe_name} saga_name={saga_name}/>
    )
}